...
export const registerUser = async (req, res) => {
  try {
    ...
    if (
      [firstname, lastname, email, username, password, confirmPassword].some(
        (field) => field?.trim() === ""
      )
    ) {
    ...
    if (password !== confirmPassword) {
      return res
        .status(401)
        .json(new ApiError(401, "Both passwords are not same"));
    }
...
export const verifyUser = async (req, res) => {
  ...
  try {
    ...
    if (user.otp.expiresAt < currentTime) {
      return res
        .status(400)
        .json(new ApiError(400, "OTP expired. Please request a new OTP"));
    }
...
export const loginUser = async (req, res) => {
  ...
  try {
    if (!password || (!email && !username)) {
      return res
        .status(400)
        .json(new ApiError(400, "Email or username and password are required"));
    }
...
export const resetPassword = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json(new ApiError(404, "Invalid or expired reset token"));
    }
